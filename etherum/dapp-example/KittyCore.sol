...
contract KittyBase is KittyAccessControl {
    ...
    /*
    KittyBase는 크립토키티 코어에서 사용하는 데이터 구조가 정의되어 있다. 
    게임 전체에서 사용하는 데이터 구조로 실제 정의되어 있는 struct는 다음과 같다. 
    그리고 solidity의 EVM에 의하면 32바이트로 채워져서 저장이 되기 때문에 
    kitty의 구조체를 보면 32바이트씩 밑에서 묶을 수 있다. 
    genes을 제외하고 나머지 요소들을 합치면 32바이트(256비트)로 묶을 수 있다. 
    여기서 이러한 최적화를 시켜 gas 비용을 줄이게 된다.
    */
