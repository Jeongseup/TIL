...
class _Test1State extends State<Test1> {
  ...
  test() async {
    Map getTopRated =
        await tmdb.v3.movies.getTopRated(language: 'ko', region: 'KR');
    myLogger.d(getTopRated);

    Map getPouplar =
        await tmdb.v3.movies.getPouplar(language: 'ko', region: 'KR');
    // myLogger.d(getPouplar);

    Map queryMovies =
        await tmdb.v3.search.queryMovies("매트릭스", language: 'ko', region: 'KR');
    // myLogger.d(queryMovies);

    String imageUrl =
        await tmdb.images.getUrl("/tva2OlLELCagFynFibcefhn2D9i.jpg") ?? "";
    // myLogger.d(imageUrl);
